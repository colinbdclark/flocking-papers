...
Flocking is a new framework for computer music composition that leverages existing technologies and ideas to create a robust, flexibile, and expressive tool. Flocking combines the UGEN pattern from many canonical computer music languages inside of WebAudio technologies to allow users to interact with existing and future web technologies. Users interact with Flocking using a declarative programming syntax. 

Various examples of web IDEs using both text and graphical environments have been demonstrated. Not only does Flocking provide users with a clear and expressive syntax but performance metrics indicate that the framework design yields superior performance.
