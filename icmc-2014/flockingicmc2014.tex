...
Significant computer music research over the past few decades has focused on the creation of specialized languages for expressing musical and time-based constructs programmatically \cite{dannenberg2002language,wang2003chuck,mccartney1996supercollider}. Though this emphasis on syntax has produced noteworthy computer music environments and useful results for use cases such as live coding, the preponderance of isolated, specialist programming languages for music and art has led to a disconnect between creative coders and the mainstream world of software developers. SuperCollider, as an example of a very successful musician-specialist programming language, provides myriad ways to express time-based concepts, yet it continues to be difficult to create polished user interfaces or to connect with web-based services and sources of data---tasks that are common in mainstream programming environments such as JavaScript. As the world of interconnected devices, sensors, and collaboration  increases, these limitations take an increasing toll on the complexity and scalability of creative coding.
...
\subsection{Declarative Interoperability}

A motivating concern for Flocking is that the prevailing emphasis on the creation of music-specific programming languages has shifted focus away from interoperability amongst tools and systems. Though the term ``interoperability" can be vague, it is used here to describe a specific concept: the ability to share a single instance of a computer music artifact (i.e. an instrument or score) {\it bidirectionally} amongst human coders, generative or transformational algorithms, and authoring or graphical tools.
...
Flocking's use of declarative programming idioms, which are discussed in more detail below, provides a foundation for supporting {\it metaprogramming} on the web. In this respect, Flocking is part of a tradition with computer music tools (often LISP-based) such as Extempore and Common Music. Flocking, however, employs this declarative approach in specific ways to facilitate the sharing of musical artifacts amongst of graphical, algorithmic, and textual tools. (THIS NEEDS TO BE REWRITTEN).
...
Flocking aims to explore and provide alternatives to this systemic distinction between signals and scheduled events. It does this by adopting a common representation of change. In Flocking, all sources of change are expressed as trees of unit generators---synths and scheduler events alike. The only difference is the rate at which these unit generators are evaluated. This single abstraction for change provides less ``conceptual surface" for newcomers to learn, and enables instruments to be used in different contexts, both at the macro and micro levels. This topic is discussed further, with examples, in section \ref{sec:Scheduling}.
