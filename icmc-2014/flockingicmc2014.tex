...
The core of the Flocking framework consists of a handful of components that provide the essential behaviour for interpreting and instantiating synths, generating samples, and scheduling changes. The most important components include:
...
\item {\it Audio Strategies}, which are pluggable audio output backends (such as the Web Audio API or ALSA on Node.js)
\item Synths, which represent instruments or collections of signal-generating logic
\item {\it Unit Generators} (ugens), which are the sample-generating primitives used to create synths
...
\caption{ A screenshot of Flocking's interactive programming environment.\label{fig:architecture}}
\end{figure}
...
\subsection{Unit and Synth Definitions}

Developers working with Flocking don't typically instantiate unit generators directly. Instead, they compose JSON objects into trees. Each node in the tree, called {\it unit generator definitions} (ugenDefs), describes a unit generator and its connection with others in the signal-processing graph. A ugenDef includes the following information:
...
Here's a simple example of a sine wave oscillator:
...
As can be seen in the code example above, unit generator types are expressed as dot-separated strings called {\it key paths} or {\it EL expressions}. These strings are bound to unit generator creator functions at instantiation time by Flocking. All type expressions refer to a global namespace hierarchy so that developers can easily contribute their own unit generator implementations (using their own namespace to avoid conflicts) and have the Flocking framework manage them in the same manner as any of the built-in types.

A collection of unit generator definitions form the basis for a {\it synth definition} (synthDef). Synth definitions describe a complete instrument to be instantiated by the Flocking framework. Synths typically include a connection to an output bus---either the speakers or a shared ``interconnect" bus. In this manner, Flocking's architecture is highly inspired by SuperCollider.

Cover:
\begin{enumerate}
\item how a synth is instantiated
\item layering--low level functions plus a framework
\item changing values on a synth--how ``cutpoint ids" are defined
\end{enumerate}
